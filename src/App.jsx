...
const CATEGORIES = ["Todos", "Party", "Estratégia", "Família", "Abstrato"];

const FILTER_OPTIONS = {
    difficulty: ["Leve", "Médio", "Pesado"],
    time: ["Rápido (<30m)", "Médio (30-60m)", "Longo (>60m)"],
    vibes: ["Competitivo", "Cooperativo", "Rir", "Pensar"]
};

// Ícones definidos como funções para evitar erro de objeto no render
const GAME_THEMES = {
  catan: {
...
const GAMES_DB = [
  { id: 1, title: "Catan", themeId: "catan", category: "Estratégia", players: "3-4", minP: 3, maxP: 4, time: "60-90m", timeVal: 60, rating: 4.8, difficulty: "Médio", vibes: ["Competitivo", "Pensar"], description: "Colete recursos, construa estradas e negocie como se não houvesse amanhã." },
  { id: 2, title: "Dixit", themeId: "dixit", category: "Party", players: "3-6", minP: 3, maxP: 6, time: "30m", timeVal: 30, rating: 4.9, difficulty: "Leve", vibes: ["Rir", "Criativo"], description: "Um jogo de narrativas oníricas onde uma imagem vale mais que mil palavras." },
  { id: 3, title: "Azul", themeId: "azul", category: "Abstrato", players: "2-4", minP: 2, maxP: 4, time: "45m", timeVal: 45, rating: 4.7, difficulty: "Médio", vibes: ["Pensar", "Competitivo"], description: "Seja um azulejista português e crie a parede mais bonita." },
  { id: 4, title: "Ticket to Ride", themeId: "ttr", category: "Família", players: "2-5", minP: 2, maxP: 5, time: "60m", timeVal: 60, rating: 4.6, difficulty: "Leve", vibes: ["Competitivo", "Pensar"], description: "Construa rotas de trem através da América do Norte." },
  { id: 5, title: "Coup", themeId: "coup", category: "Party", players: "2-6", minP: 2, maxP: 6, time: "15m", timeVal: 15, rating: 4.5, difficulty: "Leve", vibes: ["Competitivo", "Rir", "Blefe"], description: "Blefe, suborno e manipulação em um futuro distópico." },
  { id: 6, title: "Terraforming Mars", themeId: "mars", category: "Estratégia", players: "1-5", minP: 1, maxP: 5, time: "120m", timeVal: 120, rating: 4.9, difficulty: "Pesado", vibes: ["Pensar", "Estratégico"], description: "Lidere uma corporação e compete para transformar Marte." }
];
...
const MATCH_HISTORY = [
    ...
    { id: 102, gameId: 3, date: "Sáb, 15:00", winner: "Ana", location: "Luderia Central", duration: "45m", scoreboard: [{ name: "Ana", score: 85, avatar: "Ana" }, { name: "Gabriel", score: 72, avatar: "GabrielUX" }] },
    { id: 103, gameId: 5, date: "Sáb, 14:00", winner: "Pedro", location: "Luderia Central", duration: "15m", scoreboard: [{ name: "Pedro", score: "Win", avatar: "Pedro" }, { name: "Gabriel", score: "Elim", avatar: "GabrielUX" }, { name: "Ana", score: "Elim", avatar: "Ana" }, { name: "Lucas", score: "Elim", avatar: "Lucas" }] }, 
...
const Header = ({ title = "Olá, Gabriel 👋", showProfile = true }) => (
  <header className="flex justify-between items-center mb-8 px-2 mt-4">
      <div><p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">{showProfile ? "Let's Play!" : "Seu Histórico"}</p><h1 className="text-3xl font-black text-slate-800 font-jakarta tracking-tight">{title}</h1></div>
...
const SearchBar = ({ searchTerm, setSearchTerm, onOpenFilters, hasActiveFilters, placeholder = "Busque sua próxima aventura..." }) => (
...
const StatsHeader = () => (
    <div className="grid grid-cols-3 gap-3 mb-8 px-1"><div className="bg-white p-4 rounded-[28px] shadow-sm border border-slate-50 flex flex-col items-center hover:shadow-md transition-shadow"><span className="text-3xl font-black text-slate-800 font-jakarta mb-1">24</span><span className="text-[9px] uppercase tracking-widest font-bold text-slate-400 text-center">Partidas</span></div><div className="bg-gradient-to-b from-[#ff9a9e] to-[#fecfef] p-4 rounded-[28px] shadow-lg shadow-pink-200 flex flex-col items-center relative overflow-hidden text-white transform scale-105"><span className="text-3xl font-black font-jakarta mb-1 flex items-center gap-1">12 <Crown size={18} fill="white" /></span><span className="text-[9px] uppercase tracking-widest font-bold text-white/90 text-center">Vitórias</span></div><div className="bg-white p-4 rounded-[28px] shadow-sm border border-slate-50 flex flex-col items-center hover:shadow-md transition-shadow"><span className="text-3xl font-black text-slate-800 font-jakarta mb-1">48h</span><span className="text-[9px] uppercase tracking-widest font-bold text-slate-400 text-center">Jogado</span></div></div>
);
        ...
        <div className="fixed inset-0 z-[60] flex justify-center pointer-events-none">
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", stiffness: 300, damping: 30 }} className="w-full max-w-md bg-[#F4F6F9] h-full overflow-y-auto shadow-2xl pointer-events-auto relative">
                ...
                <div className="p-6 pb-24">
                    ...
                    <SettingsSection title="Geral"><SettingRow icon={User} label="Editar Perfil" color="text-blue-500" /><SettingRow icon={Bell} label="Notificações" type="toggle" value={true} color="text-orange-500" /><SettingRow icon={Moon} label="Modo Escuro" type="toggle" value={false} color="text-purple-500" /><SettingRow icon={Shield} label="Privacidade" color="text-emerald-500" /></SettingsSection>
    ...
    return (
        <button onClick={() => onClick(match)} className="w-full flex items-center gap-4 p-4 rounded-[24px] border border-slate-100 bg-white shadow-sm hover:border-[#4FACFE]/40 hover:shadow-lg transition-all text-left">
            ...
            <div className="flex-1">
                ...
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{match.date} • {match.duration}</span>
            </div>
...
const DesktopLayout = ({
    ...
    return (
        <div className="hidden lg:flex w-full">
            <div className="w-full max-w-7xl mx-auto grid grid-cols-[280px_minmax(0,1fr)_320px] gap-10 py-12 px-12">
                <div className="space-y-8">
                    <div className="bg-white/90 rounded-[32px] p-6 shadow-xl shadow-blue-100/40 border border-white/60">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1"><MapPin size={12} /> Maringá, PR</p>
                            </div>
                        </div>
                        <div className="grid grid-cols-3 gap-4 mt-6">
                            {[{ label: "Jogos", value: 42 }, { label: "Partidas", value: 128 }, { label: "Vitórias", value: 24 }].map((stat) => (
                    ...
                    <div className="bg-white/90 rounded-[32px] p-6 shadow-lg shadow-blue-100/40 space-y-4 border border-white/60">
                        <div className="flex items-center justify-between">
                            <h3 className="text-sm font-black text-slate-500 uppercase tracking-widest">Atalhos Rápidos</h3>
                            <InfoIcon className="text-slate-300" size={16} />
                        </div>
                        <div className="space-y-3">
                            <DesktopQuickAction icon={Filter} label="Filtros avançados" description="Refine sua busca" onClick={() => setShowFilters(true)} />
                            <DesktopQuickAction icon={Plus} label="Relembrar última partida" description="Ticket to Ride" gradient="from-[#ff9a9e] to-[#fecfef]" onClick={() => setSelectedMatch(MATCH_HISTORY[0])} />
                            <DesktopQuickAction icon={Settings} label="Preferências e conta" description="Notificações e plano" gradient="from-[#a18cd1] to-[#fbc2eb]" onClick={onOpenSettings} />
                        </div>
                    </div>
                ...
                <div className="space-y-8">
                    <div className="flex items-center justify-between">
                        <div>
                            <p className="text-xs font-bold uppercase tracking-[0.3em] text-slate-400">Dashboard</p>
                            <h1 className="text-4xl font-black text-slate-900 font-jakarta">Sua Ludoteca em alta definição</h1>
                        </div>
                        <button onClick={() => setShowFilters(true)} className="hidden xl:flex items-center gap-2 px-4 py-3 rounded-full border border-slate-200 text-sm font-bold text-slate-500 hover:border-[#4FACFE] hover:text-[#4FACFE] transition-colors">
                            <Filter size={18} /> Filtros
                        </button>
                    </div>
                    <SearchBar searchTerm={searchTerm} setSearchTerm={setSearchTerm} onOpenFilters={() => setShowFilters(true)} hasActiveFilters={hasActiveFilters} placeholder="Busque por título, vibe ou número de jogadores..." />
                    <HeroSection onClick={setSelectedGame} />
                    <div className="bg-white/90 rounded-[32px] p-6 shadow-xl shadow-blue-100/50 border border-white/60">
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                <h2 className="text-2xl font-black font-jakarta text-slate-800">{hasActiveFilters ? "Resultados filtrados" : "Minha coleção"}</h2>
                ...
                <div className="space-y-8">
                    ...
                    <div className="bg-white/90 rounded-[32px] p-6 shadow-xl shadow-blue-100/50 border border-white/60">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <Badge icon={Zap} label="Relâmpago" gradient="from-[#4FACFE] to-[#00F2FE]" />
                            <Badge icon={Users} label="Anfitrião" gradient="from-[#43e97b] to-[#38f9d7]" />
                            <Badge icon={TrendingUp} label="Evolução" gradient="from-[#a18cd1] to-[#fbc2eb]" />
                        </div>
                    </div>
                    <div className="bg-white/90 rounded-[32px] p-6 shadow-xl shadow-blue-100/50 border border-white/60">
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                <h3 className="text-xl font-black text-slate-800 font-jakarta">Últimas Partidas</h3>
...
export default function LudotecaHomeV14() {
  ...
  const filteredGames = useMemo(() => {
    return GAMES_DB.filter(game => {
        ...
        if (filters.time) {
            if (filters.time.includes("Rápido")) matchesTime = game.timeVal < 30;
            else if (filters.time.includes("Médio")) matchesTime = game.timeVal >= 30 && game.timeVal <= 60;
        ...
        <div className="flex-grow overflow-y-auto scrollbar-hide">
            {activeTab === 'home' && (
                <div className="p-6">
                    ...
                    <div className="mb-6 flex items-end justify-between px-2"><h2 className="text-xl font-black font-jakarta text-slate-800">{hasActiveFilters ? "Resultados Filtrados" : "Minha Coleção"}</h2><span className="text-xs font-bold text-slate-400 bg-white px-3 py-1 rounded-full shadow-sm">{filteredGames.length} Jogos</span></div>
